...
def preprocess_qwen_visual(
    ...
    source = sources[0]
    base_path = Path(source.get("data_path") or "")
    ...
    def __init__(self, processor, data_args):
        ...
        for data in dataset_list:
            ...
            if data_args.data_path:
                anno_path = data_args.data_path
            file_format = anno_path.split(".")[-1]
            if file_format == "jsonl":
                annotations = read_jsonl(anno_path)
            else:
            ...
            for ann in annotations:
                if isinstance(ann, list):
                    for sub_ann in ann:
                        sub_ann["data_path"] = data.get("data_path") or ""
                else:
                    ann["data_path"] = data.get("data_path") or ""
...
def make_supervised_data_module(processor, data_args) -> Dict:
    ...
    if data_args.eval_data_path:
        eval_args = replace(data_args, data_path=data_args.eval_data_path)
